...
dx = -0.0
yO, x = np.histogram(z_O, 32, (9.01 + dx, 34.6 + dx))
yZ, x = np.histogram(z_Zn, 32, (9.01 + dx, 34.6 + dx))
xi, yiO = smear(x[:-1], yO, 3, len(x) * 10)
xi, yiZ = smear(x[:-1], yZ, 3, len(x) * 10)
plt.plot(xi, yiZ / yiO)
# plt.plot(xi, yiO / (yiO + yiZ))
# plt.plot(xi, yiZ / (yiO + yiZ))
print((x + dx) / 50.22)
