...
def main():
    ...
    parser.add_argument('afilename', metavar='<ados-file>', type=str,
                        help="the PDOS file generated by CP2K")
    parser.add_argument('bfilename', metavar='<bdos-file>', type=str,
                        help="the PDOS file generated by CP2K")
    parser.add_argument('natoms', metavar='<number-of-atoms>', type=int,
                        help="the total number of atoms of this kind in the structure")
    ...
    with open(args.afilename, 'r') as fhandle:
    ...
    with open(args.bfilename, 'r') as fhandle:
        header = HEADER_MATCH.match(fhandle.readline().rstrip())
        if not header:
            print(("The file '{}' does not look like a CP2K PDOS output.\n"
                   "If it is indeed a correct output file, please report an issue at\n"
        ...
        header = fhandle.readline().rstrip().split()[1:]  # remove the comment directly
        header[1:3] = [' '.join(header[1:3])]  # rejoin "Eigenvalue" and its unit
        data2 = np.loadtxt(fhandle)  # load the rest directly with numpy
    ...
    with smart_open(args.output) as fhandle:
        ...
        if args.total_sum:
            for mpnt in range(nmesh):
                print(("{:16.8f}" + " {:16.8f}").format(xmesh[mpnt], sum(ymesh[mpnt, :])), file=fhandle)
        else:
            for mpnt in range(nmesh):
                print(("{:16.8f}" + " {:16.8f}"*ncols).format(xmesh[mpnt], *ymesh[mpnt, :]), file=fhandle)

        if args.total_sum:
            for mpnt in reversed(range(nmesh)):
                print(("{:16.8f}" + " {:16.8f}").format(xmesh[mpnt], -sum(ymesh2[mpnt, :])), file=fhandle)
        else:
            for mpnt in range(nmesh):
                print(("{:16.8f}" + " {:16.8f}"*ncols).format(xmesh[mpnt], *-ymesh2[mpnt, :]), file=fhandle)
